...
int main(int argc, char** argv)
{
	...
	// 照らされるキューブ用のシェーダー
	Shader cubeshader("DiffuseMap.vert", "SoftEdge.frag");
	...
	while (renderLoop)
	{
		...
		// cube
		cubeshader.use();
		cubeshader.setVec3("light.position", flyCamera.GetPositionVec().x, flyCamera.GetPositionVec().y, flyCamera.GetPositionVec().z);
		cubeshader.setVec3("light.direction", flyCamera.GetFrontVec().x, flyCamera.GetFrontVec().y, flyCamera.GetFrontVec().z);
		cubeshader.setFloat("light.cutOff", cosf(Math::ToRadians(12.5f)));
		cubeshader.setFloat("light.outerCutOff", cosf(Math::ToRadians(17.5f)));
		cubeshader.setVec3("viewPos", flyCamera.GetPositionVec().x, flyCamera.GetPositionVec().y, flyCamera.GetPositionVec().z);
